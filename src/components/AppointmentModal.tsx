...
export default function AppointmentModal({open, onClose, onSave, hairdressers, services, editAppointment, isSaving}: AddAppointmentModalProps){
                  ...
                  >
                    ...
                    {services.map((s) => (
                      <option key={s.id} value={s.id} className="bg-[#0F1623]">
                        {s.name} · {formatDuration({ hours: Math.floor(s.durationMinutes / 60), minutes: s.durationMinutes % 60 })} · {formatPrice(s.price)}
                      </option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 pointer-events-none" />
