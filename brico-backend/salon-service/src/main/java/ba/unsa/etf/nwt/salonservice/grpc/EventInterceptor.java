package ba.unsa.etf.nwt.salonservice.grpc;
...
public class EventInterceptor implements HandlerInterceptor {
    ...
    private String extractResource(String uri) {
        if (uri.contains("/services"))     return "Service";
        if (uri.contains("/hairdressers")) return "Hairdresser";
        if (uri.contains("/photos"))       return "Photo";
        if (uri.contains("/salons"))       return "Salon";
        return "Unknown";
    }
}
